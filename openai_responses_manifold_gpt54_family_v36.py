...
MODEL_PRICING_USD_PER_MTOK = {
    # Current GPT‑5.4 frontier family
    ...
    # Reasoning / thinking variants are billed like the base model they target
    "gpt-5.2-thinking-low": {"input": 1.75, "output": 14.00},
    "gpt-5.2-thinking-medium": {"input": 1.75, "output": 14.00},
    "gpt-5.2-thinking-high": {"input": 1.75, "output": 14.00},
    "gpt-5.2-thinking-xhigh": {"input": 1.75, "output": 14.00},
...
IMAGE_MODEL_PRICING_USD = {
    # Medium-quality 1024x1024 image generation prices.
    "gpt-image-1.5": {"1024x1024": 0.034},
    "gpt-image-1": {"1024x1024": 0.042},
    "gpt-image-1-mini": {"1024x1024": 0.011},
}

# Conservative flat image estimate (USD) when we cannot see size/quality.
IMAGE_ESTIMATED_COST_PER_IMAGE_USD = 0.04
...
ROUTER_DEBUG_LINE_RE = re.compile(r"(?im)^gpt-5-auto routing:[^\n]*$")
...
class CompletionsBody(BaseModel):
    ...
    def normalize_model(self) -> "CompletionsBody":
        """Normalize model: strip 'openai_responses.' prefix and map pseudo-models."""
        ...
        aliases = {
            # GPT-5.4 thinking family (pseudo → real model + reasoning effort)
            "gpt-5-thinking": ("gpt-5.4", "medium"),
            "gpt-5-thinking-low": ("gpt-5.4", "low"),
            "gpt-5-thinking-medium": ("gpt-5.4", "medium"),
            "gpt-5-thinking-high": ("gpt-5.4", "high"),
            "gpt-5-thinking-xhigh": ("gpt-5.4", "xhigh"),
            "gpt-5.4-thinking": ("gpt-5.4", "medium"),
            "gpt-5.4-thinking-low": ("gpt-5.4", "low"),
            "gpt-5.4-thinking-medium": ("gpt-5.4", "medium"),
            "gpt-5.4-thinking-high": ("gpt-5.4", "high"),
            "gpt-5.4-thinking-xhigh": ("gpt-5.4", "xhigh"),
            # Legacy aliases kept working, but retargeted to the current frontier model.
            "gpt-5-thinking-minimal": ("gpt-5.4", "low"),
            "gpt-5.1-thinking": ("gpt-5.4", None),
            "gpt-5.1-thinking-none": ("gpt-5.4", "none"),
            "gpt-5.1-thinking-low": ("gpt-5.4", "low"),
            "gpt-5.1-thinking-medium": ("gpt-5.4", "medium"),
            "gpt-5.1-thinking-high": ("gpt-5.4", "high"),
            ...
            # Legacy GPT-5 mini thinking aliases now target GPT-5.4 mini.
            "gpt-5-thinking-mini": ("gpt-5.4-mini", "medium"),
            "gpt-5-thinking-mini-minimal": ("gpt-5.4-mini", "low"),
            # GPT-5 Pro aliases
            "gpt-5-pro-high": ("gpt-5-pro", "high"),
            # Placeholder router
            "gpt-5-auto": ("gpt-5.4", None),
...
class ResponsesBody(BaseModel):
    ...
    def from_completions(
        ...
        **extra_params,
        ...
        # "gpt-5.4-pro", "gpt-5-auto") so the assistant can answer with the
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        # 2) Models
        MODEL_ID: str = Field(
            default="gpt-5-auto, gpt-5.4, gpt-5.4-pro, gpt-5.4-mini, gpt-5.4-nano, gpt-5-thinking-low, gpt-5-thinking-medium, gpt-5-thinking-high, gpt-5-thinking-xhigh, gpt-5-pro, gpt-5-codex, gpt-5.3-codex, gpt-4.1, gpt-4.1-mini, gpt-4.1-nano, gpt-4o, o3",
            description=(
                "Comma separated OpenAI model IDs. Each ID becomes a model entry in WebUI. "
                "Supports official OpenAI model IDs plus a small set of pseudo IDs: "
                "gpt-5-auto, "
                "gpt-5-thinking-low, "
        ...
        GPT5_AUTO_ROUTER_MODEL: str = Field(
            ...
            description=(
                ...
                "gpt-5-auto should route to (for example, gpt-4.1-nano or gpt-5.4-nano)."
            ),
        )
        ...
        GPT5_AUTO_ROUTER_DEBUG: bool = Field(
            default=False,
            description=(
                "If true, expose gpt-5-auto routing debug info indicating whether "
                "the small-model router or the heuristic fallback was used."
            ),
        )
        GPT5_AUTO_ROUTER_DEBUG_DISPLAY: Literal["toast", "under_response"] = Field(
            default="under_response",
            description=(
                "When GPT5_AUTO_ROUTER_DEBUG is enabled, show the routing decision "
                "either as a toast notification or appended under the assistant response."
            ),
        )
        # 3) Reasoning & summaries
        REASONING_SUMMARY: Literal["auto", "concise", "detailed", "disabled"] = Field(
            default="disabled",
            description="REQUIRES VERIFIED OPENAI ORG. Visible reasoning summary (auto | concise | detailed | disabled). Applied only on supported reasoning models and ignored otherwise. Docs: https://platform.openai.com/docs/api-reference/responses/create#responses-create-reasoning",
        )
        ...
        ENABLE_WEB_SEARCH_TOOL: bool = Field(
            default=False,
            description="Enable OpenAI's built-in 'web_search_preview' tool when supported by the selected model. NOTE: This appears to disable parallel tool calling. Read more: https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses",
        )
    ...
    async def pipe(
        ...
        gpt5_auto_debug_notice = ""
        if openwebui_model_id and openwebui_model_id.endswith(".gpt-5-auto"):
            ...
            if valves.GPT5_AUTO_ROUTER_DEBUG:
                actual_label = responses_body.model
                if effort:
                    pretty_effort = {
                        "minimal": "minimal",
                        "low": "low",
                        ...
                        "xhigh": "xhigh",
                        "none": "no",
                    }.get(str(effort).lower(), str(effort).lower())
                    actual_label = f"{actual_label} ({pretty_effort} reasoning)"
                gpt5_auto_debug_notice = (
                    f"gpt-5-auto routing: {routed_model}{router_debug_tag} -> "
                    f"{actual_label}"
                )
        ...
        if gpt5_auto_debug_notice:
            __metadata__["gpt5_auto_debug_text"] = gpt5_auto_debug_notice
        model_family = re.sub(r"-\d{4}-\d{2}-\d{2}$", "", responses_body.model)

        # GPT-5 pro only supports high reasoning effort.
        if model_family in {"gpt-5-pro"}:
        ...
        if (
            ...
            and (
                (responses_body.reasoning or {}).get("effort", "").lower() != "low"
            )
        ):
            ...
            if model:
                ...
                if params.get("function_calling") != "native":
                    ...
                    if supports_function_calling:
                        ...
                        update_result = Models.update_model_by_id(openwebui_model_id, form)
    ...
    async def _run_streaming_loop(
        ...
        # Catch any exceptions during the streaming loop and emit an error
            ...
            router_debug_text = (metadata.get("gpt5_auto_debug_text") or "").strip()
            if router_debug_text:
                if valves.GPT5_AUTO_ROUTER_DEBUG_DISPLAY == "under_response":
                    assistant_message = self._append_router_debug_once(
                        assistant_message, router_debug_text
                    )
                else:
                    await self._emit_notification(
                        event_emitter,
                        content=router_debug_text,
                        level="info",
                    )
    ...
    async def _run_nonstreaming_loop(
        ...
        try:
            ...
            if router_debug_text:
                if valves.GPT5_AUTO_ROUTER_DEBUG_DISPLAY == "under_response":
                    final_text = self._append_router_debug_once(
                        final_text, router_debug_text
                    )
                else:
                    await self._emit_notification(
                        event_emitter,
    ...
    @staticmethod
    def _append_router_debug_once(text: str, debug_line: str) -> str:
        """Append the GPT-5-auto routing debug line once near the end."""
        if not debug_line:
            return text

        cleaned = ROUTER_DEBUG_LINE_RE.sub("", text).strip()
        if not cleaned:
            return debug_line
    ...
    @staticmethod
    def _extract_router_target(raw_text: str, allowed_targets: set[str]) -> str:
        """Best-effort extraction of a model ID from router output text."""
        ...
        if first_token in allowed_targets:
            return first_token
        ...
        if isinstance(parsed, str):
            ...
            for key in ("model", "target", "choice", "route"):
        ...
        for target in sorted(allowed_targets, key=len, reverse=True):
            if re.search(
                rf"(?<![a-z0-9_.-]){re.escape(target)}(?![a-z0-9_.-])", lowered
            ):
                return target
    ...
    async def _route_gpt5_auto(
        ...
        if not text:
            ...
            return "gpt-5.4-nano", self._debug_tag(valves, "fallback")
        ...
        def heuristic_route() -> str:
            ...
            pro_requested = any(
                ...
                for phrase in [
                    ...
                    "use gpt-5-pro",
                    "use the pro model",
                    "run with gpt-5.4-pro",
                    "run with gpt-5.2-pro",
                    "run with gpt-5-pro",
                ]
            )
            ...
            # 3.1 Direct to GPT-5.4 Pro
            if pro_requested or char_len > 4000 or ultra_hard_hit:
                return "gpt-5.4-pro"

            # 3.2 Thinking models (gpt-5.4 + reasoning.effort)
            if reasoning_on or heavy_hit or "explain your reasoning" in lower:
                # User hints about effort level.
                if "xhigh" in lower or "extra high" in lower:
                    return "gpt-5-thinking-xhigh"
                if "low" in lower or "minimal" in lower:
                    return "gpt-5-thinking-low"
                if (
                    "high" in lower
            ...
            # 3.3 Medium / long prompts → gpt-5.4
            if char_len > 800 or word_len > 160:
                return "gpt-5.4"

            # 3.4 Short, truly simple prompts → gpt-5.4-nano
            if (
                char_len < 80
                and word_len < 15
                and not heavy_hit
            ...
            # 3.5 Default general assistant choice → gpt-5.4-mini
            return "gpt-5.4-mini"
        ...
        system_instructions = (
            ...
            "- Use gpt-5.4-nano for very short, simple prompts that are not deeply\n"
            "  technical, not code-heavy, and do not require detailed reasoning.\n"
            "- Use gpt-5.4-mini for normal chat, everyday questions, and lightweight\n"
            "  coding where lower latency and lower cost matter.\n"
            "- Use gpt-5.4 when the prompt is long or somewhat complex, where extra\n"
            "  reasoning, robustness, or instruction-following is helpful, but full\n"
            "  step-by-step reasoning is not strictly necessary.\n"
            "- Use gpt-5-thinking-low / gpt-5-thinking / gpt-5-thinking-high / gpt-5-thinking-xhigh when\n"
            ...
            "- Prefer gpt-5-thinking-low for light reasoning, gpt-5-thinking for\n"
            "  most reasoning tasks, gpt-5-thinking-high for hard ones, and\n"
            "  gpt-5-thinking-xhigh for the hardest reasoning requests.\n"
            "- Use gpt-5.4-pro ONLY when the user\n"
        ...
        for candidate in (router_model, "gpt-4.1-nano", "gpt-5.4-nano"):
            candidate = (candidate or "").strip()
            if candidate and candidate not in router_candidates:
                router_candidates.append(candidate)

        last_router_reason = "fallback"

        for candidate_model in router_candidates:
            ...
            try:
                ...
                routed_raw = str(response.get("output_text") or "").strip()
                if not routed_raw:
                    text_parts: list[str] = []
                    for item in response.get("output", []):
                        if item.get("type") != "message":
                ...
                if routed_raw:
                    routed = self._extract_router_target(routed_raw, allowed_targets)

                if routed in allowed_targets:
                    tag = "router" if candidate_model == router_model else f"router:{candidate_model}"
                    return routed, self._debug_tag(valves, tag)

                last_router_reason = "fallback:empty-output"
                if routed_raw:
                    last_router_reason = "fallback:invalid-output"

                if hasattr(self, "logger"):
                    self.logger.debug(
                        "gpt-5-auto router returned no valid target. model=%s raw=%r",
                        candidate_model,
                        routed_raw,
                    )

            except Exception as exc:
                last_router_reason = "fallback:api-error"
                if hasattr(self, "logger"):
                    self.logger.exception(
                        "gpt-5-auto router model failed with %s; trying fallback router if available",
                        candidate_model,
                        exc_info=exc,
                    )
        ...
        return fallback_choice, self._debug_tag(valves, last_router_reason)
...
def _normalize_model_for_pricing(model: str) -> str:
    ...
    in Open WebUI (e.g. ``openai_responses.gpt-5.4``) and finally to a safe
    default of ``gpt-5.4``.
    """
    ...
    if model in chat_aliases:
        return chat_aliases[model]

    # Strip known prefixes like "openai_responses." etc.
    for sep in (".", "/"):
        if sep in model:
            candidate = model.split(sep)[-1]
            if candidate in chat_aliases:
                return chat_aliases[candidate]
...
def estimate_response_cost_usd(
    ...
    if not usage:
        if include_image_costs and _is_image_model(model):
            return float(IMAGE_ESTIMATED_COST_PER_IMAGE_USD)
        return 0.0
    ...
    if include_image_costs:
        ...
        if image_count <= 0 and (
            _is_image_model(model) or _is_image_model(usage_model)
        ):
            image_count = 1

        if image_count:
            cost += IMAGE_ESTIMATED_COST_PER_IMAGE_USD * image_count
...
def format_cost_summary(
    ...
    *,
    ...
    image_cost = 0.0
    if image_count:
        image_cost = IMAGE_ESTIMATED_COST_PER_IMAGE_USD * image_count
    ...
    def _format_model_label() -> str:
        ...
        mapping = {
            "gpt-5-thinking-low": "gpt-5.4",
            "gpt-5-thinking": "gpt-5.4",
            "gpt-5-thinking-medium": "gpt-5.4",
            "gpt-5-thinking-high": "gpt-5.4",
            "gpt-5-thinking-xhigh": "gpt-5.4",
            "gpt-5-thinking-minimal": "gpt-5.4",
            "gpt-5-thinking-mini": "gpt-5.4-mini",
            "gpt-5-thinking-mini-minimal": "gpt-5.4-mini",
            "gpt-5.4-thinking": "gpt-5.4",
            "gpt-5.4-thinking-low": "gpt-5.4",
            "gpt-5.4-thinking-medium": "gpt-5.4",
            "gpt-5.4-thinking-high": "gpt-5.4",
            "gpt-5.4-thinking-xhigh": "gpt-5.4",
            "gpt-5-pro": "gpt-5-pro",
            "gpt-5.4": "gpt-5.4",
        ...
        reasoning_effort = {
            ...
            "gpt-5-thinking-minimal": "low",
            "gpt-5.4-thinking": "medium",
            "gpt-5.4-thinking-low": "low",
            "gpt-5.4-thinking-medium": "medium",
            "gpt-5.4-thinking-high": "high",
            "gpt-5.4-thinking-xhigh": "xhigh",
            "gpt-5-thinking-mini": "medium",
            "gpt-5-thinking-mini-minimal": "low",
    ...
    if include_image_costs and image_count:
        include_note = (
            f" (includes ~${image_cost:.6f} for {image_count} {plural} "
            f"at ~${IMAGE_ESTIMATED_COST_PER_IMAGE_USD:.2f} each; image cost is an estimate)"
        )

    cost_line = (
        f"[approx cost this reply ({label_model}): ${cost_this:.6f} USD"
        f"{include_note}"
    )
    if chat_id:
        cost_line += f" | approx total for this chat: ${cumulative:.6f} USD"
    cost_line += "]"

    return (cost_line, cost_this, cumulative)

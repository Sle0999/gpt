...
class Pipe:
    ...
    async def pipe(
        ...
        # Add image_generation tool when enabled by WebUI features and
        # supported by the selected model family. Without this, OpenAI returns
        # "tool not found" when the frontend requests image generation.
        if (
            model_family in FEATURE_SUPPORT["image_gen_tool"]
            and (
                features.get("image_generation", False)
                or features.get("image_gen", False)
            )
        ):
            responses_body.tools = responses_body.tools or []
            responses_body.tools.append({"type": "image_generation"})

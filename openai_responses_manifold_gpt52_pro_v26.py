...
def normalize_tool_registry(tools: Any) -> dict[str, dict[str, Any]]:
    """Return a lookup map that supports both internal IDs and spec.name keys.

    Open WebUI ``__tools__`` can be keyed by internal IDs that differ from
    the model-visible function name.  This normalizer ensures runtime execution
    can resolve tools by either key.
    """
    if not tools:
        return {}
    ...
    if isinstance(tools, dict):
        ...
        iterable = [(None, tool_cfg) for tool_cfg in tools]

    for original_key, tool_cfg in iterable:
        if not isinstance(tool_cfg, dict):
            continue

        spec_name = tool_cfg.get("spec", {}).get("name")

        if original_key is not None:
            normalized[str(original_key)] = tool_cfg

        if spec_name:
            normalized[str(spec_name)] = tool_cfg

    return normalized


def extract_tool_names(tools: Any) -> list[str]:
    """Extract function names from mixed tool schemas for concise debug logs."""
    names: list[str] = []
    if not tools:
        return names
    ...
    for item in iterable:
        ...
        elif isinstance(item.get("spec"), dict) and item["spec"].get("name"):
            names.append(str(item["spec"]["name"]))
    return names


def merge_response_tools(*tool_sets: Any) -> list[dict[str, Any]]:
    """Merge tool definitions while deduplicating function tools by name."""
    merged: list[dict[str, Any]] = []
    for tool_set in tool_sets:
        merged.extend(ResponsesBody.transform_tools(tool_set, strict=False))
    ...
    for tool in merged:
        if not isinstance(tool, dict):
            continue
        key = (
            f"function:{tool.get('name')}"
            if tool.get("type") == "function"
            else f"type:{tool.get('type', 'unknown')}"
        )
        dedup[key] = tool
...
MODEL_PRICING_USD_PER_MTOK = {
    # GPT‑5.2 family (standard API pricing)
    "gpt-5.2": {"input": 1.25, "output": 10.00},
    "gpt-5.2-chat-latest": {"input": 1.25, "output": 10.00},
    # Reasoning / thinking variants are billed like the base GPT‑5.2 family
    "gpt-5.2-thinking-low": {"input": 1.25, "output": 10.00},
    "gpt-5.2-thinking-medium": {"input": 1.25, "output": 10.00},
    "gpt-5.2-thinking-high": {"input": 1.25, "output": 10.00},
    "gpt-5.2-thinking-xhigh": {"input": 1.25, "output": 10.00},
...
class CompletionsBody(BaseModel):
    ...
    def normalize_model(self) -> "CompletionsBody":
        ...
        aliases = {
            # GPT-5.2 Thinking family (pseudo → real model + reasoning effort)
            "gpt-5-thinking": ("gpt-5.2", "medium"),
            "gpt-5-thinking-low": ("gpt-5.2", "low"),
            "gpt-5-thinking-medium": ("gpt-5.2", "medium"),
            ...
            "gpt-5-thinking-minimal": ("gpt-5.2", "low"),
            "gpt-5.1-thinking": ("gpt-5.2", None),
            "gpt-5.1-thinking-none": ("gpt-5.2", "none"),
            "gpt-5.1-thinking-low": ("gpt-5.2", "low"),
            "gpt-5.1-thinking-medium": ("gpt-5.2", "medium"),
            "gpt-5.1-thinking-high": ("gpt-5.2", "high"),
            # GPT-5 mini thinking (unchanged)
            "gpt-5-thinking-mini": ("gpt-5-mini", "medium"),
            "gpt-5-thinking-mini-minimal": ("gpt-5-mini", "minimal"),
            # GPT-5 Pro (always high effort) → GPT-5.2 Pro
            "gpt-5-pro": ("gpt-5.2-pro", "high"),
            "gpt-5-pro-high": ("gpt-5.2-pro", "high"),
            # Placeholder router
            ...
            "gpt-5.1-chat-latest": ("gpt-5.2-chat-latest", None),
...
class ResponsesBody(BaseModel):
    ...
    def from_completions(
        ...
        **extra_params,
        ...
        # "gpt-5.2-pro", "gpt-5-auto") so the assistant can answer with the
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        REASONING_SUMMARY: Literal["auto", "concise", "detailed", "disabled"] = Field(
            default="disabled",
            description="REQUIRES VERIFIED OPENAI ORG. Visible reasoning summary (auto | concise | detailed | disabled). Works on gpt-5.2, o3, o4-mini; ignored otherwise. Docs: https://platform.openai.com/docs/api-reference/responses/create#responses-create-reasoning",
        )
        ...
        ENABLE_IMAGE_TOOL: bool = Field(
            default=False,
            description="Enable fallback generate_image tool execution (OpenAI Images API) when only request-body tool definitions are provided without callable bindings.",
        )

        DEBUG_TOOLS: bool = Field(
            default=False,
            description="Emit concise debug logs for tool discovery and runtime resolution.",
        )
    ...
    async def pipe(
        ...
        # GPT-5.2 Pro variants do not support streaming; enforce non-streaming mode
        # regardless of whether it was explicitly chosen or selected by the
        # router.
        if responses_body.model in {"gpt-5.2-pro"}:
        ...
        # Enforce reasoning.effort for GPT-5.2 Pro: API only accepts 'high' for this model.
        if model_family in {"gpt-5.2-pro"}:
        ...
        tools_for_exec = normalize_tool_registry(__tools__)
        body_tool_registry = normalize_tool_registry(body_tools)
        for k, cfg in body_tool_registry.items():
            if isinstance(cfg, dict) and callable(cfg.get("callable")):
                tools_for_exec[k] = cfg

        # Merge tool definitions from Open WebUI runtime bindings and request body.
        if model_family in FEATURE_SUPPORT["function_calling"]:
            responses_body.tools = merge_response_tools(__tools__, body_tools)

            fallback_image_name = "generate_image"
            has_body_generate_image = fallback_image_name in extract_tool_names(body_tools)
            has_exec_generate_image = fallback_image_name in tools_for_exec

            if has_body_generate_image and not has_exec_generate_image:
                if valves.ENABLE_IMAGE_TOOL:
                    fallback_cfg = {
                        "spec": {
                        ...
                        "callable": self._build_generate_image_fallback_callable(valves),
                    }
                    tools_for_exec[fallback_image_name] = fallback_cfg
                    self.logger.info(
                        "Installed fallback callable for tool '%s' from request body definitions.",
                        fallback_image_name,
                    )
                else:
                    responses_body.tools = [
                        ...
                        if not (
                            ...
                            and t.get("name") == fallback_image_name
                        )
                    ]
                    self.logger.info(
                        "Dropped request tool '%s' because ENABLE_IMAGE_TOOL is disabled and no runtime callable exists.",
                        fallback_image_name,
                    )
        ...
        if valves.DEBUG_TOOLS:
            registry_keys = sorted(list(tools_for_exec.keys()))
            entry_samples = []
            for key in registry_keys[:8]:
                cfg = tools_for_exec.get(key)
                if isinstance(cfg, dict):
                    entry_samples.append(
                        {
                            "key": key,
                            "keys": sorted(list(cfg.keys()))[:8],
                            "spec_name": cfg.get("spec", {}).get("name") if isinstance(cfg.get("spec"), dict) else None,
                            "has_callable": callable(cfg.get("callable")),
                        }
                    )
            self.logger.debug(
                "Tool debug: request_has_tools=%s request_tool_names=%s registry_keys=%s registry_samples=%s",
                bool(body_tools),
                extract_tool_names(body_tools),
                registry_keys,
                redact(entry_samples),
            )
        ...
        if responses_body.stream:
            ...
            return await self._run_streaming_loop(
                responses_body, valves, __event_emitter__, __metadata__, tools_for_exec
            )
        else:
    ...
    async def _run_streaming_loop(
        ...
        try:
            for loop_idx in range(valves.MAX_FUNCTION_CALL_LOOPS):
                ...
                if calls:
                    function_outputs = await self._execute_function_calls(calls, tools, debug_tools=valves.DEBUG_TOOLS)
    ...
    def _build_generate_image_fallback_callable(self, valves: "Pipe.Valves") -> Callable[..., Awaitable[str]]:
        """Build async fallback callable for generate_image tool."""

        async def _fallback_generate_image(
            prompt: str,
            size: str = "1024x1024",
            n: int = 1,
            **_: Any,
        ) -> str:
            return await self._call_openai_images_api(
                prompt=prompt,
                size=size,
                n=n,
                api_key=valves.API_KEY,
        ...
        return _fallback_generate_image

    async def _call_openai_images_api(
        self,
        *,
        prompt: str,
        size: str,
        ...
        """Generate image(s) via OpenAI Images API and return markdown output."""
        ...
        async with self.session.post(url, json=payload, headers=headers) as resp:
            ...
            if resp.status >= 400:
                body_excerpt = text_body[:600]
                raise RuntimeError(
                    f"Image generation failed (HTTP {resp.status}): {body_excerpt}"
                )
            ...
            try:
                ...
                raise RuntimeError(
                    "Image generation failed: invalid JSON response from images endpoint"
                )

        images = data.get("data") or []
        markdown_parts: list[str] = []
        for idx, image in enumerate(images, start=1):
            b64 = (image or {}).get("b64_json") if isinstance(image, dict) else None
            if not b64:
                continue
            markdown_parts.append(f"![generated image {idx}](data:image/png;base64,{b64})")
    ...
    async def _execute_function_calls(
        ...
        *,
        ...
        def _resolve_tool_cfg(call_name: str) -> dict[str, Any] | None:
            tool_cfg = tools.get(call_name)
            if tool_cfg:
                return tool_cfg
            ...
            for candidate in tools.values():
                if not isinstance(candidate, dict):
                    continue
                if candidate.get("spec", {}).get("name") == call_name:
                    return candidate

            available_spec_names = sorted(
                {
                    candidate.get("spec", {}).get("name")
                    for candidate in tools.values()
                    if isinstance(candidate, dict) and candidate.get("spec", {}).get("name")
                }
            )
            log.debug(
                "Tool lookup failed for '%s'. Available spec names: %s",
                call_name,
        ...
        def _make_task(call):
            if debug_tools:
                available_keys = sorted(list(tools.keys()))[:40]
                sample_shapes = []
                for k in available_keys[:8]:
                    cfg = tools.get(k)
                    if isinstance(cfg, dict):
                ...
                log.debug(
                    "Executing tool call name=%s available_registry_keys=%s sample_shapes=%s",
                    call.get("name"),
                    available_keys,
                    redact(sample_shapes),
                )

            tool_cfg = _resolve_tool_cfg(call["name"])
    ...
    async def _route_gpt5_auto(
        ...
        def heuristic_route() -> str:
            ...
            # 3.2 Thinking models (gpt-5.2 + reasoning.effort)
        ...
        system_instructions = (
            ...
            "- Use gpt-5.2-pro ONLY when the user\n"
...
def _normalize_model_for_pricing(model: str) -> str:
    ...
    in Open WebUI (e.g. ``openai_responses.gpt-5.2``) and finally to a safe
    default of ``gpt-5.2``.
    """
...
def format_cost_summary(
    ...
    *,
    ...
    def _format_model_label() -> str:
        ...
        mapping = {
            "gpt-5-thinking-low": "gpt-5.2",
            "gpt-5-thinking": "gpt-5.2",
            "gpt-5-thinking-medium": "gpt-5.2",
            "gpt-5-thinking-high": "gpt-5.2",
            "gpt-5-thinking-xhigh": "gpt-5.2",
            "gpt-5-thinking-minimal": "gpt-5.2",
...
async def _self_test_tool_registry_resolution() -> None:
    """Minimal smoke test for function-call resolution by spec.name."""
    ...
    normalized = normalize_tool_registry(raw_tools)
    outputs = await Pipe._execute_function_calls(calls, normalized)
    if outputs[0].get("output") != "ok":
        raise AssertionError(f"Expected tool output 'ok', got: {outputs}")


if __name__ == "__main__":
    asyncio.run(_self_test_tool_registry_resolution())
